...
async function notifyAdmins(env, title, body, data) {
  try {
    ...
    // Booking persistence must never fail just because push delivery is unavailable.
  }
}
...
async function api(request, env, ctx) {
  ...
  if (request.method === "GET" && path === "/v1/health") {
    return json({ ok: true, service: "bwd-cloudflare", database: true, push_configured: !!serviceAccount(env) });
  }
