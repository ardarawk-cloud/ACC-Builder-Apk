...
# Owner launch must always land on the booking dashboard. Accept already-patched sources.
if 'buildShell(); showHome();' in s:
    s = s.replace('buildShell(); showHome();', 'adminMode=true; buildShell(); showOwnerInbox();', 1)
elif 'adminMode=true; buildShell(); showOwnerInbox();' not in s:
    raise SystemExit('owner launch anchor not found')

# Owner shell: no customer bottom navigation.
...
elif 'private void showAdmin(){showOwnerInbox();}' not in s:
...
if book_start >= 0 and book_end >= 0:
    s = s[:book_start] + '    private void showAdminBookings(){showOwnerInbox();}\n\n' + s[book_end:]
elif 'private void showAdminBookings(){showOwnerInbox();}' not in s:
...
required = [
    ...
    'BwdCloud.showAdminEnrollment(this)',
    'BOOKING INBOX',
    'OPEN BOOKING',
